...
/// and OpenUSD registers the file formats whose plugins actually loaded. They
...
enum class FormatState {
    ...
    /// extension and OpenUSD registered a file format for it. `open` will
    /// dispatch on it.
    available,
    /// The composition resolves the capability but OpenUSD registered nothing.
    /// The plugin is installed and did not load; this is the state that
    /// explains a `capability_unavailable` failure for a format the runtime
    /// metadata says is present, and the `provider` fields name what to look
    /// at.
    not_loaded,
    /// OpenUSD registered a file format the composition never asked for. This
...
struct Format {
    ...
    /// OpenUSD registered a file format for this extension.
...
/// Join what a composition declares with what OpenUSD registered.
///
/// `registered` is the extension list of the loaded process, which only the
/// OpenUSD lane can obtain; taking it as an argument is what keeps the rule
/// that turns two lists into one answer testable on a machine with no OpenUSD,
/// and it is what `formats()` in the OpenUSD lane calls once it has asked.
/// Entries are lowercased, deduplicated, and sorted by extension, so two
/// reports of one runtime compare as text.
