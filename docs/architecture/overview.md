...
| Path | Responsibility |
| --- | --- |
| `targets/<target>/composition.toml` | Declares a target, required capabilities, immutable candidate artifacts, and explicit providers. |
| `targets/<target>/lock.json` | Pins resolution and artifact identities for reproducible composition. |
| `targets/<target>/metadata.json` | Generated machine-readable target identity, immutable identities, components, and capabilities. |
...
| `evidence/<release>-<target>.json` | Preserves the concise accepted-release result and immutable runtime identities. |
...
`targets/<target>/metadata.json` is the machine-readable description of the
composed target; each manifest gets a sibling document. It is generated by
`tools/runtime_metadata.py` from committed
